...
- [x] Expand real-hardware SecureStore/TPM parity so certified TPM-backed systems consistently use the TPM anchor path when the TPM probe succeeds; otherwise the boot path explicitly falls back to the non-TPM recovery flow instead of silently misreporting the state.
...
- [x] Implement `seal_secret()` / `unseal_secret()` abstraction (TPM-backed when the TPM provider is enabled; otherwise the API reports unsupported/busy conditions and the caller uses the recovery path) (sources: [backups/todo_archive_2026-03-25/TODO_S_LIST.md](backups/todo_archive_2026-03-25/TODO_S_LIST.md#L51))
...
- [x] Add TPM‑accelerated sealing for AI metadata (optional) (implemented as `QK::SecureStore::writeTpmSealedBlob()` / `readTpmSealedBlob()` which seal a per-blob content key via TPM policy when the boot-time TPM provider is available, and otherwise do not activate the TPM path). (sources: [backups/todo_archive_2026-03-25/CITADEL_TASKFLOW_TODO.md](backups/todo_archive_2026-03-25/CITADEL_TASKFLOW_TODO.md#L38))
