...
- [ ] (MVP) VFS: add per-file `role_flag` in metadata (not filename/path-based)
...
- [ ] (MVP) Crypto FS: define role → encryption tier mapping
...
- [ ] (Later) Crypto FS: implement per-tier key schedule
...
- [ ] (MVP → Later) Enforce access as “role AND keys”
...
- [ ] (Later) Rotation: rewrap tier keys without decrypting all files
