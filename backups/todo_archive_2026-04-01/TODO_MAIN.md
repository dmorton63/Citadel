...
- [ ] VFS: add per-file `role_flag` metadata (tamper-evident; not filename/path-based)
  - Store `role_flag` in file metadata (enforced by VFS; covered by metadata hashing so tampering is evident):
    - `ROLE_EVERYONE`
    - `ROLE_USER`
    - `ROLE_ADMIN`
    - `ROLE_SYSTEM`
...
- [ ] Crypto FS: define role → encryption tier mapping (role-anchored encryption)
  - Initial mapping (final tier count can change, but `ROLE_ADMIN -> Level 3` and `ROLE_SYSTEM -> Level 4` are fixed by design intent):
    | role_flag | encryption tier | intent |
    |---|---:|---|
    | `ROLE_EVERYONE` | Level 1 | device-bound “baseline” readable by all |
    | `ROLE_USER` | Level 2 | user-bound secrets |
    | `ROLE_ADMIN` | Level 3 | admin clearance |
    | `ROLE_SYSTEM` | Level 4 | system clearance |
    | `ROLE_SC` | Level 5 | SC isolation (unreadable unless SC mediates) |
    | `ROLE_PROTECTED` | Level 6 | special sealed partitions/areas |
- [ ] Crypto FS: derive per-tier key material (role-bound keys)
  - Derive per-role/per-tier keys from `SST`, `UMK`, `VRK`, `role_id`, `version`.
- [ ] VFS+Crypto: enforce “role AND keys” for access
  - Require BOTH: correct role (policy check) AND correct key material (cryptographic gate).
  - `ROLE_SC` data must remain unreadable unless SC mediates.
- [ ] Rotation: rewrap per-role keys without decrypting all files
  - Goal: rotate `SST` and rewrap per-role/per-tier keys without decrypting every file (rewrap-without-decrypting).
