...
namespace CQL {
    ...
    DropTableCommand SQLParser::ParseDropTable(const std::string& query) {
        ...
        if (tableNameStart == std::string::npos) {
            return {false, "Invalid DROP TABLE syntax", "", false};
        }
        ...
        // Look for "IF EXISTS" (case-insensitive)
        std::string remaining = query.substr(tableNameStart);
        std::string remainingUpper = remaining;
        for (char& c : remainingUpper) c = toupper(c);

        if (remainingUpper.find("IF") == 0) {
            size_t ifPos = tableNameStart;
            tableNameStart += 2; // Skip "IF"
            ...
            // Check for "EXISTS"
            remaining = query.substr(tableNameStart);
            remainingUpper = remaining;
            for (char& c : remainingUpper) c = toupper(c);

            if (remainingUpper.find("EXISTS") == 0) {
                ifExists = true;
                tableNameStart += 6; // Skip "EXISTS"
            } else {
                // "IF" without "EXISTS" is invalid
                return {false, "Invalid syntax: expected 'IF EXISTS'", "", false};
            }
        }
        ...
        if (tableNameStart == tableNameEnd) {
            return {false, "No table name found", "", false};
        }

        std::string tableName = query.substr(tableNameStart, tableNameEnd - tableNameStart);

        return {true, "", tableName, ifExists};
    }
    ...
    SelectCommand SQLParser::ParseSelect(const std::string& query) {
        ...
        std::vector<std::string> whereLogicalOps;
        ...
        if (wherePos != std::string::npos) {
            ...
            // Parse compound WHERE clause (AND/OR support)
            if (!whereColumn.empty()) {
                // Add first condition
                whereConditions.push_back(WhereCondition(whereColumn, whereValue, whereOperator));

                // Look for AND/OR keywords
                size_t searchPos = condStart;
                while (searchPos < query.length()) {
                    // Skip whitespace
                    while (searchPos < query.length() && isspace(query[searchPos])) {
                        searchPos++;
                    }

                    // Check for ORDER BY, LIMIT, OFFSET, or end of query
                    std::string upperRest = query.substr(searchPos);
                    for (auto& c : upperRest) c = toupper(c);

                    if (upperRest.find("ORDER") == 0 || upperRest.find("LIMIT") == 0 || 
                        upperRest.find("OFFSET") == 0 || upperRest.find(";") == 0 ||
                        searchPos >= query.length()) {
                        break; // No more WHERE conditions
                    }

                    // Look for AND/OR
                    std::string logicalOp;
                    if (upperRest.find("AND") == 0 && 
                        (searchPos + 3 >= query.length() || isspace(query[searchPos + 3]))) {
                        logicalOp = "AND";
                        searchPos += 3;
                    } else if (upperRest.find("OR") == 0 && 
                               (searchPos + 2 >= query.length() || isspace(query[searchPos + 2]))) {
                        logicalOp = "OR";
                        searchPos += 2;
                    } else {
                    ...
                    // Skip whitespace after AND/OR
                    while (searchPos < query.length() && isspace(query[searchPos])) {
                        searchPos++;
                    }

                    // Parse next condition (column op value)
                    WhereCondition nextCond;

                    // Parse column name
                    size_t nextColStart = searchPos;
                    while (searchPos < query.length() && 
                           (isalnum(query[searchPos]) || query[searchPos] == '_' || query[searchPos] == '.')) {
                        searchPos++;
                    }
                    nextCond.column = query.substr(nextColStart, searchPos - nextColStart);
                    ...
                    // Parse operator
                    if (searchPos < query.length()) {
                        if (query[searchPos] == '=') {
                            nextCond.op = "=";
                            searchPos++;
                        }
                        else if (searchPos + 1 < query.length() && query[searchPos] == '!' && query[searchPos + 1] == '=') {
                            nextCond.op = "!=";
                            searchPos += 2;
                        }
                        else if (searchPos + 1 < query.length() && query[searchPos] == '<' && query[searchPos + 1] == '>') {
                    ...
                    // Parse value
                    bool nextInQuotes = false;
                    if (searchPos < query.length() && (query[searchPos] == '\'' || query[searchPos] == '"')) {
                        nextInQuotes = true;
                        searchPos++;
                    }

                    size_t nextValStart = searchPos;
                    while (searchPos < query.length()) {
                        if (nextInQuotes && (query[searchPos] == '\'' || query[searchPos] == '"')) {
                            break;
                        }
                        if (!nextInQuotes && (isspace(query[searchPos]) || query[searchPos] == ';')) {
                            break;
                        }
                        searchPos++;
                    }

                    nextCond.value = query.substr(nextValStart, searchPos - nextValStart);

                    // Skip closing quote if needed
                    if (nextInQuotes && searchPos < query.length()) {
                        searchPos++;
                    }

                    // Add condition and logical operator
                    whereLogicalOps.push_back(logicalOp);
                    whereConditions.push_back(nextCond);
                }
            }
        }
        ...
        cmd.whereConditions = whereConditions;      // Compound WHERE support
        cmd.whereLogicalOps = whereLogicalOps;      // AND/OR operators
