...
namespace CQL {
    ...
    bool Database::CreateTable(const std::string& tableName, const std::vector<ColumnInfo>& columns) {
        ...
        std::strncpy(tableEntry.name, tableName.c_str(), sizeof(tableEntry.name) - 1);
        tableEntry.name[sizeof(tableEntry.name) - 1] = '\0';
        ...
        for (const auto& col : columns) {
            ColumnDef colDef;
            std::strncpy(colDef.name, col.name.c_str(), sizeof(colDef.name) - 1);
            colDef.name[sizeof(colDef.name) - 1] = '\0';
    ...
    bool Database::AlterTableAddColumn(const std::string& tableName, const std::string& columnName,
        ...
        ColumnDef colDef;
        std::strncpy(colDef.name, columnName.c_str(), sizeof(colDef.name) - 1);
        colDef.name[sizeof(colDef.name) - 1] = '\0';
    ...
    std::string Database::SelectJoinRows(const std::string& leftTableName, const std::string& rightTableName,
        ...
        // Apply ORDER BY (similar to SelectRows)
        if (!orderByColumn.empty()) {
            ...
            // Find order column in selected columns
            int orderByIndex = -1;
            for (size_t i = 0; i < selectedColumnIndices.size(); i++) {
                if (mergedColumns[selectedColumnIndices[i]].name == resolvedOrderByColumn) {
                    orderByIndex = static_cast<int>(i);
                    break;
                }
            }
            ...
            ColumnType orderColType = mergedColumns[selectedColumnIndices[orderByIndex]].type;
